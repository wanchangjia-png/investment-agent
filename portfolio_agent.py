...
SECTOR_OUTLOOK = {
    ...
    "半导体/芯片": "国产替代和AI算力需求驱动长期增长，但短期受地缘政治和周期波动影响；关注库存去化节奏和先进制程突破。",
    "石油/能源": "油价受地缘政治和OPEC+产量政策影响波动较大，中国海油作为国内最大海上油气生产商，受益于油价高位和产量增长，分红稳定，防御属性较强。",
    "军工电子/元器件": "军工行业受国防预算增长和装备升级驱动，火炬电子在MLCC和特种陶瓷材料领域有技术优势；关注军工订单节奏和成本端压力。",
}
...
def fetch_prices():
    ...
    # 股票价格 — 优先从持仓 Excel 读代码（网页编辑添加的股票自动生效），硬编码 STOCK_CODES 作为补充
    holdings = load_portfolio()
    stock_names = {}
    for h in holdings:
        if h["类别"] == "股票" and h.get("代码"):
            stock_names[h["代码"]] = h["名称"]
    # 补充硬编码映射（覆盖同名）
